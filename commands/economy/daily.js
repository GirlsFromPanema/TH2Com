...
module.exports = {
	name: 'daily',
	description: 'Collect a reward every day',
	usage: '',
	example: '',
	category: 'Economy',
	...
	async execute({ message, settings, prefix }) {
		...
		if (data?.cooldowns?.DAILY > Date.now()) {
			return message.channel.send(`:x: | You must wait **${ms(data.cooldowns.DAILY - Date.now())}**`);
		}

		const earning = randomNum(250, 50);
		await addMoney(message.author.id, earning);
		return message.channel.send(`✅ | You just collected <:th2:827279307574738964> **${earning}** Coins!`);
	}
};
