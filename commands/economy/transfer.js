...
module.exports = {
	...
	async execute({ message, settings, client, prefix }) {
		...
		if (user?.cooldowns?.TRANSFER > Date.now()) {
			return message.channel.send(`:x: | You must wait **${ms(user.cooldowns.TRANSFER - Date.now())}**`);
		}
		...
		if (mText.content > 5000) return msg.edit(':x: | You can\'t transfer more than **5000**', { embed: null });
		...
		return msg.edit(`${emojis.tick} | You gave <:th2:827279307574738964> **${mText.content}** Coins to **${member.user.username}**`, { embed: null });
	}
};
