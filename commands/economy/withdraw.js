...
module.exports = {
	...
	async execute({ message, prefix, settings, client }) {
		...
		if (m.content.toLowerCase() === 'all') {
			...
			if (!settings.economyChannelID) return message.channel.send(`:x: | You must setup an economy channel in order to use this command!\n\n\`\`\`${prefix}seteconomy <mentionChannel>\`\`\``, { embed: null });
			if (user && user.balance && user.balance > 0) {
				await addMoney(message.author.id, user.balance);
				return msg.edit(`✅ | Successfully withdrawn <:th2:827279307574738964> **${user.balance}** Coins from the bank!`, { embed: null });
			}
		...
		if (user && user.balance && user.balance > 0) {
			...
			if (num > user.balance) return msg.edit(':x: | You do not have enough balance to withdraw!', { embed: null });

			await addMoney(message.author.id, num);
			return msg.edit(`${emojis.tick} | Successfully withdrawn <:th2:827279307574738964> **${num}** Coins from the bank!`, { embed: null });
		}

		await m.delete();
		return msg.edit(':x: | You do not have any balance!', { embed: null });
	}
};
