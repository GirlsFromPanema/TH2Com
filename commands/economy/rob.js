...
module.exports = {
	...
	async execute({ message, args, client }) {
		...
		if (!member) return msg.edit(':x: | You must mention someone!', { embed: null });

		const data = await economyUser.findOne({ userID: message.author.id });
		if (data?.cooldowns?.ROB > Date.now()) {
			return message.channel.send(`:x: | ${message.author} You are on cooldown, you must wait: **${ms(data.cooldowns.ROB - Date.now())}**`);
		}
		...
		if (robbed === 0) {
			return msg.edit('This user has <:th2:827279307574738964> **0** Coins in cash!', { embed: null });
		}

		await robMoney(member.id, robbed);
		await addMoney(message.author.id, robbed);
		return msg.edit(`${message.author} Successfully robbed <:th2:827279307574738964>**${robbed}** Coins from **${member.username}**`, { embed: null });
	}
};
