const jobs = ['House wife', 'Programmer', 'Builder', 'McDonald\'s employee', 'Law enforcer', 'Lawyer', 'Banker', 'Cleaner', 'Discord Mod, Computer Student, Architect, Dentist, Co-Worker'];
...
module.exports = {
	...
	async execute({ message, settings, prefix }) {
		...
		if (data?.cooldowns?.WORK > Date.now()) {
			return message.channel.send(`:x: | You must wait **${ms(data.cooldowns.WORK - Date.now())}**`);
		}

		const earning = randomNum(5000, 1500);
