...
module.exports = {
	...
	async execute({ message, settings, prefix }) {
		...
		if (settings.economyChannelID && message.channel.id !== settings.economyChannelID) return message.channel.send(`:x: | You may only use this command in <#${settings.economyChannelID}>`);
		...
		const earning = win ? 15000 : -15000;
		await addMoney(message.author.id, earning);

		if (win) {
			return message.channel.send('You committed a crime and got <:th2:827279307574738964> **15000** Coins!');
		}
		return message.channel.send(':x: | You got caught and fined <:th2:827279307574738964> **15000** Coins!');
	}
};
