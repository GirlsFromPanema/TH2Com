...
module.exports = {
	...
	async execute({ message, settings, prefix }) {
		...
		if (data?.cooldowns?.ROLL > Date.now()) {
			return message.channel.send(`:x: | You must wait **${ms(data.cooldowns.ROLL - Date.now())}**`);
		}

		const earning = randomNum(2500, 500);
		await addMoney(message.author.id, earning);

		return message.channel.send(`You rolled <:th2:827279307574738964> **${earning}** Coins!`);
	}
};
