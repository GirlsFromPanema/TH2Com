...
module.exports = {
	...
	async execute({ message, settings, prefix }) {
		...
		if (data?.cooldowns?.BEG > Date.now()) {
			return message.channel.send(`:x: | You must wait **${ms(data.cooldowns.BEG - Date.now())}**`);
		}

		const earning = randomNum(5000, 500);
		await addMoney(message.author.id, earning);
		return message.channel.send(`Someone gave you <:th2:827279307574738964> **${earning}** Coins!`);
	}
};
