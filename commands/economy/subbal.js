...
module.exports = {
	...
	async execute({ message, args, client }) {
		...
		if (!user) return message.channel.temp(':x: | You must mention someone');
		await message.delete().catch(() => null);

		const bal = Number(args[1]) || 0;
		await addMoney(user.id, bal);
		return message.channel.temp(`Removed <:th2:827279307574738964> **${bal}** Coins from **${user.tag}'s** balance!`);
	}
};
